...
function dbs_post_actions() {
	...
	case 'pull' :
		try {
			...
			$result = dbs_post( $_REQUEST['url'], 'dbs_pull', array( 'table_prefix'=>dbs_get_table_prefix(), 'secret' => $tokens[$_REQUEST['url']], 'url' => $_REQUEST['url'] ) );
	...
	case 'push' :
		...
		if ( $sql = dbs_get_mysql_dump() ) {
			try {
				$sql = dbs_merge_network_plugins($sql);

				file_put_contents(realpath(dirname(__FILE__)).DIRECTORY_SEPARATOR.'last_push_result.sql', $sql);

				//send post request with secret and SQL data
				$table_prefix = dbs_get_table_prefix();
...
function dbs_pull() {
	...
	$sql = dbs_get_mysql_dump();
	$sql = dbs_merge_network_plugins($sql);
	// error_log($sql,0);
	$sql = dbs_sql_unlocalize_tables( $sql, $_REQUEST['table_prefix'] );

	// error_log("RESULT: ".$sql);
	file_put_contents(realpath(dirname(__FILE__)).DIRECTORY_SEPARATOR.'last_pull_result.sql', $sql);
